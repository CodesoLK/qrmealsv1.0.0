...
@font-face {
    ...
    src: url('{{ asset('fonts/Srilankan.ttf') }}');
}
...
<div class="container-fluid mt--7">
        ...
        <div class="row">
            ...
            <div class="col">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('menu-designer.update', $template->id) }}" class="submit-template-form form-group mb-2 d-flex justify-content-between">
                            @csrf
                            @method('PUT')
                            ...
                            <fieldset class="fieldset-template">
                                ...
                                <div class="row mb-2">
                                    <div class="col">
                                        ...
                                        <input type="number" class="form-control layout-width" readonly/>
                                    </div>
                                    <div class="col">
                                        <label>Template Height (px)</label>
                                        <input type="number" class="form-control layout-height" readonly/>
                                    </div>
                                </div>
    ...
    <script>
        ...
        $('.add-layer-image').on('click', function(e) {
            ...
            myOffcanvas.addEventListener('hidden.bs.offcanvas', event=>{
                ...
                if(clicked_outside) {
                    ezBSAlert({
                        type: "confirm",
                        messageText: "You are about to close the layer tab.",
                        headerText: "Do you want to apply the Layer?",
                        alertType: "primary"
                    }).done(function (e) {
                        if(e) {
                            applyThisImageLayer()
            ...
            $(`.layer-image-${LAYER}`).on('change', function(e) {
                ...
                reader.onloadend = function () {
                    var temp_img = document.querySelector('.bg-cropper')
                    $('.bg-cropper').attr('src', reader.result)
                    var cropper = new Cropper(temp_img, {
                        minContainerWidth: INITIAL_WIDTH,
                        minContainerHeight: INITIAL_HEIGHT,
                        movable: false,
                        autoCrop: false
                    ...
                    $('.crop-modal-btn').on('click', function(e) {
                        if(cropper) {
                            var b64img = cropper.getCroppedCanvas().toDataURL("image/png")
                            addImageToTemplate(id, b64img, LAYER_IMAGE_WIDTH)
                            cropper.destroy()
                            cropper = null
                        }
                        if(cropModal) {
            ...
            $(`.layer-image-${id}`).on('change', function(e) {
                ...
                reader.onloadend = function () {
                    ...
                    $('.crop-modal-btn').on('click', function(e) {
                        if(cropper) {
                            var b64img = cropper.getCroppedCanvas().toDataURL("image/png")
                            $(`.layer-image-${id} img`).attr('src', b64img)
                            cropper.destroy()
                            cropper = null
                        }
                        if(cropModal) {
            ...
            myOffcanvas.addEventListener('hide.bs.offcanvas', event=>{
                if(clicked_outside) {
                    ezBSAlert({
                        type: "confirm",
                        messageText: "You are about to close the layer tab.",
                        headerText: "Do you want to apply changes to this Layer?",
                        alertType: "primary"
                    }).done(function (e) {
                        if(e) {
                            applyThisImageLayer()
        ...
        function appendTextLayer(id, value="", color="#ffffff", fontSize=1, fontWeight="normal", fontStyle="normal", textDecoration="none", fontFamily="inherit") {
            ...
            var layout = `<div class="offcanvas offcanvas-end" data-bs-scroll="true" tabindex="-1" id="offcanvasRight${id}" aria-labelledby="offcanvasRightLabel">
                            <div class="offcanvas-header">
                                <h5 class="offcanvas-title" id="offcanvasRightLabel">Layer ${id}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                            </div>
                            <div class="offcanvas-body">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea type="text" class="form-control" id="layer-text-${id}">${value}</textarea>
                                </div>
                                <div class="form-group mb-3">
            ...
            $(`#layer-text-${id}`).on('keyup', function(e) {
                ...
                var enteredTextEncoded = enteredText.replace(/\n/g, '<br>\n');
                var position = {x:0,y:0}
                var style = {
                    color,
                    fontSize,
                ...
                addTextToTemplate(id, enteredTextEncoded, position, style)
            })
